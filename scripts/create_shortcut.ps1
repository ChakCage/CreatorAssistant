...
if (-not $Executable) {
    $Executable = if ($Edition -eq 'developer') {
        Join-Path $Root 'dist\CreatorAssistant-Developer\CreatorAssistant-Developer.exe'
    } else {
...
$ShortcutName = if ($Edition -eq 'developer') { 'Creator Assistant Developer' } else { 'Creator Assistant' }
