...
def select_proxy_video(formats: Iterable[VideoFormat], maximum_height: int = 720) -> VideoFormat:
    candidates = [
        fmt
        for fmt in formats
...
def build_format_plan(formats: List[VideoFormat], proxy_height: int = 720) -> FormatPlan:
    ...
    proxy_video = select_proxy_video(formats, proxy_height)
    ...
    return FormatPlan(
        ...
        proxy_requires_transcode=not is_reaper_compatible(proxy_video, proxy_audio),
