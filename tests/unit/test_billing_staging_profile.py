...
def test_commercial_staging_uses_separate_appdata_qsettings_and_credentials(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("CREATOR_ASSISTANT_EDITION", "commercial")
    monkeypatch.setattr(
        build_info_module, "current_build_info",
        lambda: SimpleNamespace(
            license_backend_profile="staging", edition="commercial",
            build_variant="staging", channel="beta",
        ),
    )
    assert config_root().name == "CommercialStaging"
    assert local_data_root().name == "CommercialStaging"
    assert qsettings_application_name() == "CreatorAssistantCommercialStaging"
